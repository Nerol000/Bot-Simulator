...
public class Environment {
    ...
    // The s-tap length is sampled from a normal distribution each hit instead of being a
    // fixed count, so bot2's combo timing jitters tick-to-tick. A purely mechanical, fixed
    // rhythm is something bot1 could overfit a hard counter to; the noise forces it to learn
    // a policy robust to a range of re-engage timings rather than one exact cadence.
    private static final double S_TAP_MEAN = 2.5;
    ...
    void updatebot2() {
        ...
        if (bot2.attackCharge < chargeBeforeSwing) {
            ...
            bot2StapTicks = (int) Math.round(Math.max(0.0, Math.min(S_TAP_MAX, sampled)));
        }
    }
    ...
    public void reset() {
        ...
        // Start each episode in the chase phase, with no carried-over s-tap.
        bot2StapTicks = 0;
    }
